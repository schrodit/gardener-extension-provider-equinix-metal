...
func (e *ensurer) EnsureKubeletConfiguration(_ context.Context, _ gcontext.GardenContext, kubeletVersion *semver.Version, new, _ *kubeletconfigv1beta1.KubeletConfiguration) error {
	...
	setFeatureGateIfSupported(new.FeatureGates, "VolumeSnapshotDataSource", kubeletVersion.String())
	setFeatureGateIfSupported(new.FeatureGates, "CSINodeInfo", kubeletVersion.String())
	setFeatureGateIfSupported(new.FeatureGates, "CSIDriverRegistry", kubeletVersion.String())
...
func setFeatureGateIfSupported(featureGates map[string]bool, featureGate, version string) error {
	isSupported, err := kubernetesutils.IsFeatureGateSupported(featureGate, version)
	if err != nil {
		return err
	}
	if isSupported {
